...
Triage each memorable statement into one of four paths: **auto-save**, **unconfirmed save**, **`_pending.md`**, or **ask-immediately**. Operate on the shared vault at `$GINARR_VAULT_ROOT/wiki/` — this is the owner-facing store they see in Obsidian. It is **separate from your private per-session auto-memory**; do not confuse the two.
...
| **Low / ambiguous** | Speculation, thinking out loud, idle reflection | **Append block to `wiki/_pending.md`.** Do *not* create a file under `wiki/<type>/`. Reviewed later via `/review`. |
...
|---|---|---|
| `user` | `wiki/user/` | Facts about the owner. ("speaks Russian and English", "has a dog named Rex") |
| `feedback` | `wiki/feedback/` | Rules for how to work with the owner. ("don't summarize at the end", "plan docs stay in Russian") |
| `project` | `wiki/projects/` | Ongoing initiatives with a lifecycle. ("marathon training — target Oct 2026") |
| `reference` | `wiki/reference/` | Pointers to external systems / people / places. ("bugs tracked in Linear INGEST", "ob sync daemon lives in ~/OpenClaw") |
| `decision` | `wiki/decisions/` | Point-in-time choices with rationale. ("chose Postgres over MySQL — need range types") |
...
6. **Dedup — search before write.** Grep the whole `wiki/` tree for the topic keyword (name, tag, subject). Use Bash: `grep -rli "<keyword>" "$GINARR_VAULT_ROOT/wiki/"` plus variants.
7. **Existing match?** → Read it. Judge: compatible (merge into the body, update `updated:` date) vs. contradictory → Conflict protocol.
8. **No match?** → `mkdir -p "$GINARR_VAULT_ROOT/wiki/<dir>/"` and Write a new file with full frontmatter (template below).
...
- `N >= 5` **and** `$GINARR_VAULT_ROOT/wiki/.pending_notified` does **not** exist → send one short Telegram message to the owner: `Накопилось N кандидатов в /review — разберёшь?` (match the owner's recent language; default to Russian). Then create `.pending_notified` so the ping does not repeat at every subsequent capture while the queue stays above the threshold.
...
- **Medium-confidence save** (`status: unconfirmed`) → react 💾 **and** send one short reply: `Saved as unconfirmed: wiki/<type>/<file>.md` (path only; do not paraphrase the content).
