...
5. **Frontmatter completeness** — required fields, `type` from the recognised list, `aliases` includes the slug.
6. **Stale `updated:`** — `updated:` more than 30 days older than the latest fact's `[[<date>]]` anchor.
...
- The owner's `_owner.md` is exempt from the orphan check (it's a hub, but never absent inbound traffic in a healthy vault). It is also exempt from the "stale updated vs latest fact" check (`_owner.md` uses topical sections, not a flat `## Facts` list).
