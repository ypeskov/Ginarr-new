...
- **`recall`** — read-side memory skill: on retrospective questions, greps `$GINARR_VAULT_ROOT/notes/` first, then `logs/summaries/`, only then drills into a specific day's `logs/.../*.jsonl`, cites the source in the reply. Never writes. Authoritative doc: [`.claude/skills/recall/SKILL.md`](../../.claude/skills/recall/SKILL.md); operator doc: [`recall.md`](recall.md).
