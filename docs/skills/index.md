...
- **`capture`** — write-side memory skill: triages a user statement into auto-save, unconfirmed save, `_pending.md`, or ask-immediately; writes to `$GINARR_VAULT_ROOT/wiki/<type>/<snake_case>.md`. Authoritative doc: [`.claude/skills/capture/SKILL.md`](../../.claude/skills/capture/SKILL.md); operator doc: [`capture.md`](capture.md).
...
- **`lint-indexes`** — walks a directory tree and ensures every folder has an `index.md` listing its files and subdirectories. Dry-run by default; `--apply` writes. Never rewrites or reorders existing index content — only appends missing entries. Authoritative doc: [`.claude/skills/lint-indexes/SKILL.md`](../../.claude/skills/lint-indexes/SKILL.md); operator doc: [`lint-indexes.md`](lint-indexes.md).
